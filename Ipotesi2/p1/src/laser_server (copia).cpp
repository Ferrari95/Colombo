...
double mdbo = 1.5;														// distanza minima fra oggetti (min dist between objects)
double mdbv = 1.5;														// distanza minima fra vertici (min dist between vertexes)

double fd1 = 0.2;														// fixed distances:
double fd2 = 0.6;														// per posizionamento nuovi vertici


enum section_type {in_range, out_range};
...
class LaserAction {
	...
	public:
		...
		void execute(const p1::laserGoalConstPtr& goal) {				// è la funzione di callback di questo server
			...
			if(laser_scan_.ranges[0] == laser_scan_.range_max)
				actual_section->type = out_range;
			else
				actual_section->type = in_range;
			
			//sections.push_back(actual_section);
			
			for(int i = 1; i < laser_scan_.ranges.size() - 2; i++) {
				if(actual_section->type == out_range) {
					if(laser_scan_.ranges[i] != laser_scan_.range_max) {
						actual_section->sx_index = i;
						actual_section->sx = 0;
						...
						if(laser_scan_.ranges[i + 1] == laser_scan_.range_max) {
							next_section->type = out_range;
							next_section->dx = 0;
						}
						else {
							next_section->type = in_range;
							next_section->dx = 1;
						}
				...
				else if(i < laser_scan_.ranges.size() - 2) {
					if(laser_scan_.ranges[i] == laser_scan_.range_max) {
						actual_section->sx = 1;
			...
			for(int i = 0; i < sections.size(); i++) {
				std::cout << "#" << i << " " << sections[i]->type << " " << sections[i]->sx_index << " " <<  sections[i]->dx_index << "\n";
			}
			...
			*/
			...
			f.push_back(laser_scan_.ranges.size());
			...
			/*
			int count = 0;
			for(int j = 0; j < laser_scan_.ranges.size(); j++) {
				if(f[count] == j) {
					std::cout << "XXXXXXX" << count << " ";
					count++;
				}
				std::cout << laser_scan_.ranges[j] << " ";
			}
			std::cout << "\n";		
			*/
